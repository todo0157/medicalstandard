...
const postal_code_service_1 = __importDefault(require("../services/postal-code.service"));
...
router.get('/search', auth_middleware_1.authenticate, async (req, res, next) => {
    try {
        ...
        if (isPostalCode) {
            try {
                const postalResults = await postal_code_service_1.default.searchByPostalCode(query.trim());
                ...
                const clientId = config_1.env.NAVER_MAP_CLIENT_ID;
                const clientSecret = config_1.env.NAVER_MAP_CLIENT_SECRET;
                // Address 형식으로 변환
...
3. 네트워크 연결 상태 확인
        ...
        if (error instanceof Error) {
            return res.status(500).json({
                ...
                details: config_1.env.NODE_ENV === 'development' ? error.message : undefined,
            });
        }
        return next(error);
    }
...
// 역지오코딩 (좌표 → 주소)
router.get('/reverse', auth_middleware_1.authenticate, async (req, res, next) => {
    try {
...
router.post('/geocode', auth_middleware_1.authenticate, async (req, res, next) => {
    try {
