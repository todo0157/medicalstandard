...
interface PostalCodeRecord {
  ...
  eupmyeon: string; // 읍면
  eupmyeonEnglish: string;
  roadNameCode: string;
  roadName: string; // 도로명
  roadNameEnglish: string;
  buildingMainNumber: string; // 건물번호본번
  buildingSubNumber: string; // 건물번호부번
  buildingName: string; // 시군구용건물명
  beopjeongdong: string; // 법정동명
  haengjeongdong: string; // 행정동명
  jibunMain: string; // 지번본번
  jibunSub: string; // 지번부번
}
...
class PostalCodeService {
  ...
  private cache: Map<string, AddressResult[]> = new Map();
  ...
  constructor() {
    ...
    possiblePaths.push(
      path.join(cwd, 'search_number'), // 프로젝트 루트 또는 /app에서 실행 시 (가장 일반적)
      path.join(cwd, '..', 'search_number'), // server 디렉토리에서 실행 시
      path.join('/app', 'search_number'), // Docker 환경에서 실행 시 (절대 경로)
    );
   ...
   */
  private parseLine(line: string): PostalCodeRecord | null {
   ...
   */
  private recordToAddress(record: PostalCodeRecord): AddressResult {
    // 도로명 주소 구성
   ...
   * 특정 파일에서 우편번호 검색
   */
  private searchInFile(filePath: string, postalCode: string): AddressResult[] {
   ...
   */
  private searchInFileStream(filePath: string, postalCode: string): AddressResult[] {
    const results: AddressResult[] = [];
    const seen = new Set<string>();
   ...
   * 우편번호로 주소 검색
   */
  async searchByPostalCode(postalCode: string): Promise<AddressResult[]> {
    // 우편번호 형식 검증 (5자리 숫자)
