...
- **프로필 이미지 스토리지 최적화 (S3 연동)**:
  ...
  - Base64 인코딩 이미지를 S3에 업로드하고 DB에는 Public URL만 저장하도록 서버 로직 변경 (`profile.routes.ts`)
  - 기존 DB에 저장된 Base64 이미지를 S3로 자동 이전하는 마이그레이션 스크립트(`migrate-images-to-s3.ts`) 구현 및 실행
  - 프론트엔드 코드 수정 없이 서버 측 변경만으로 최적화 달성 (투명한 마이그레이션)
  - `s3-uploader` IAM 사용자 및 버킷 정책 설정을 통해 보안성 확보
