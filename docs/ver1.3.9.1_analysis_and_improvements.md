...
1. **회원가입 & 인증**
   ...
   - JWT 기반 세션 관리
   - **한의사 인증 필드**: DB 스키마에 `isPractitioner`, `certificationStatus` 필드 존재
   - **인증 상태 업데이트 API**: `/profiles/:id/certification` 엔드포인트 존재
...
2. **한의사 찾기 & 예약**
   ...
   - 슬롯 조회 및 예약
   - 예약 취소 기능
   - **주소 검색 완전 구현**: 네이버 지도 API 통합 (ver1.3.8)
   - **우편번호 검색**: 로컬 우편번호 DB 통합 (ver1.3.9)
   - **세부 주소 입력**: 동/호수 등 상세 주소 입력 기능
...
4. **프로필 & 진료 기록**
   ...
   - 진료 기록 조회
   - 진료 기록 생성 (한의사만 가능, 권한 검증)
   - 의료진 인증 상태 관리 (DB 스키마 및 API만 존재)
...
### 1. **한의사 인증 프로세스 미구현**
**현재 상태**: DB 스키마와 API만 존재, 실제 인증 워크플로우 없음
**문제점**:
- 한의사 인증 신청 UI 없음
- 자격증/문서 업로드 기능 없음
- 관리자 승인 프로세스 없음
...
```
1단계: 한의사 인증 신청 UI
  - 프로필 화면에 "한의사 인증 신청" 버튼 추가
  - 인증 신청 화면 구현
  - 자격증/문서 업로드 기능
  - 인증 상태 표시 (none/pending/verified)
...
### 7. **검색 및 필터링 기능 부족**
**현재 상태**: 한의사 이름/전문분야/병원명 검색 및 위치 기반 검색 가능
**개선 계획**:
```
1단계: 고급 검색
  - 전문 분야 필터
...
- **주소 검색**: 네이버 지도 API (이미 구현됨, ver1.3.8)
...
- `/patients` (환자 관리)
- `/payments` (결제)
- `/addresses` (주소 검색) - ✅ 이미 구현됨 (ver1.3.8)
...
- **우편번호 검색 기능** (ver1.3.9)
  ...
  - 세부 주소 입력 기능
- **배포 환경 지원** (ver1.3.9.1)
  - Docker 배포 시 우편번호 DB 자동 포함
  - Git LFS 통합

### ⚠️ 여전히 미구현인 항목
- 한의사 인증 프로세스 (DB 스키마만 존재)
- 프로필 이미지 스토리지 개선
- 알림 시스템
- "생활" 탭
- 환자 관리 기능
